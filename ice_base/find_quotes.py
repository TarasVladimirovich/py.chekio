...
def find_quotes(a):
    print(a.split('\"')[1::2])
    indexes = [m.start(0) for m in re.finditer('"', a)]
    return [(a[indexes[i]+1:indexes[i+1]]) for i in range(0, len(indexes), 2)]

# find_quotes = lambda a: a.split('\"')[1::2]

if __name__ == '__main__':
    print("Example:")
    print(find_quotes('"Greetings""Greetings""Greetings""Greetings"'))
